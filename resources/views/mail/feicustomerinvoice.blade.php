...
@php
$TotalCost=$TotalCost+($podet->Quantity*$podet->unitcost);
@endphp
<tr>
<td>{{ $podet ->ProductCode}}</td>
<td>{{ $podet ->ProductName}}</td>
<td>{{ $podet ->Quantity}}</td>
<td>{{ $podet ->unitcost}}</td>
<td>{{ $podet ->Options}}</td>
<td>@php echo('$'.round($podet->Quantity*$podet->unitcost,2));@endphp
</tr>
                        @endforeach

@php
$ShippingCost = ($orderinfo[0]->TotalShippingCost+$poinfo[0]->ShippingCost)*$checkshippingcharge;
$TotalCost=$TotalCost+(($orderinfo[0]->TotalShippingCost+$poinfo[0]->ShippingCost)*$checkshippingcharge);
